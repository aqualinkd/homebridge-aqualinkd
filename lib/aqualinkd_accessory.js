...
AqualinkdAccessory.prototype = {
  ...
  /*
  NSF: Moved logic to handleMQTTMessage, due to homebridge crap. It seems to cache calls to setTemperature so conversion needs to be done prior
  ...
  */
  setTemperature : function(value, callback, context) {
    this.platform.log("Aqualinkd_accessory.setTemperature '"+this.id+"' "+value+" context="+context);
...
/*
    // NSF: Convert F to C at display time (when HomeKit reads the value)
    // This allows future config setting for user's temperature display preference (F or C)
    var displayValue = this.value;
    if (this.type === Constants.adDeviceValue && this.id.slice(-2) != "_f") {
      displayValue = Utils.degFtoC(this.value);
      displayValue = Utils.parseAccessoryValue(this.type, displayValue);
      this.platform.log("Aqualinkd_accessory.getTemperature '"+this.id+"' converted value for displaying to "+displayValue);
    }
  */  
    // NSF come back and check this, we should not need the if test, but no harm leaving it.
    if (typeof callback === "function") {
      // Old code that returned unconverted value:
      callback(null, this.value);
      //callback(null, displayValue);
    }
    return;
  },
  ...
  handleMQTTMessage : function(topic, message, callback) {
    ...
    //this.platform.log("MQTT message '%s' check accessory ID '%s'",topic, this.id); 
    ...
    if (this.id == topic) {
      ...
      if (this.type === Constants.adDeviceSwitch || this.type === Constants.adDevicePrgSwitch) {
        ...
        // NSF Have to have logic here for conversion due to homebridge crappy implementation.
        if (this.type === Constants.adDeviceValue && this.id.slice(-2) != "_f") {
          value = Utils.degFtoC(Utils.parseAccessoryValue(this.type, message));
        } else {
          value = Utils.parseAccessoryValue(this.type, message);
        }
        //value = Utils.parseAccessoryValue(this.type, message);
        service = this.getService(Service.TemperatureSensor);
        characteristic = this.getCharacteristic(service, Characteristic.CurrentTemperature);
        this.platform.log("Aqualinkd_accessory.handleMQTTMessage '"+this.id+"' message="+message+" value="+value);
