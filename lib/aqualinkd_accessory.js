  ...
  handleMQTTMessage : function(topic, message, callback) {
    ...
    if (this.id == topic) {
      ...
      if (this.type === Constants.adDeviceSwitch || this.type === Constants.adDevicePrgSwitch) {
        ...
        this.platform.log("Aqualinkd_accessory.config.degc="+this.platform.isUserDeviceDegC+" VSP_as_Fan="+this.platform.isVSPasFanEnabled);
        
        
        if (this.type === Constants.adDeviceValue && this.id.slice(-2) != "_f" && this.platform.isUserDeviceDegC == false) {
